...
#
# Disclaimer: this is synthetic data generated for examples and tests. It does
# not describe a real company, business unit, financial result, or employee
# population. No external data producer is represented.
...
bu_quarterly <- expand_grid(
  ...
  mutate(
    ...
    revenue_brl = annual_rev / 4 * season_mult * (1 + rnorm(n(), 0, 0.02)),
    cogs_brl = revenue_brl * cogs_ratio * (1 + rnorm(n(), 0, 0.015)),
    ebitda_brl = revenue_brl *
      ebitda_margin +
      rnorm(n(), 0, abs(revenue_brl) * 0.015),
    headcount = as.integer(round(
      annual_rev * 1000 / rev_per_head * (1 - 0.015 * abs(year_offset))
    ))
  ) |>
  transmute(
    quarter,
