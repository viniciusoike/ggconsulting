...
# not describe real clients, survey respondents, survey results, or a real NPS
# program. No external data producer is represented.
...
client_nps <- expand_grid(
  quarter = quarters,
  segment = segment_params$segment
) |>
  left_join(segment_params, by = "segment") |>
  group_by(segment) |>
  mutate(
    t = (row_number() - 1) / (n() - 1),
    nps = as.integer(round(
      nps_start + (nps_end - nps_start) * t + rnorm(n(), 0, noise_sd)
    )),
